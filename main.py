...
class AttentionFusion(nn.Module):
    def __init__(self, feature_dim):
        ...
        self.attention = nn.Sequential(
            ...
            nn.Sigmoid()  # Changed from Softmax to Sigmoid
        )
...
if __name__ == "__main__":
    ...
    predict_sequence_all_frames(
        image_folder="data/test/26",
        pose_file="data/normalized_2d_poses/26_2D_annotation_normalized.npy",
        model=model,
        transform=val_transform,
        device=device,
        output_file="output/predicted_labels.npy"
    )